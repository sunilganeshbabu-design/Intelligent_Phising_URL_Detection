...
from .feature_extractor import FEATURE_METADATA, FEATURE_NAMES
...
def get_or_create_explainers(model):
    ...
    if _shap_explainer is None:
        try:
            ...
            print(f"[-] Fallback SHAP Explainer due to: {e}")
    ...
    if _lime_explainer is None:
        try:
            ...
            print(f"[-] LIME Explainer init error: {e}")
            _lime_explainer = None
...
def explain_prediction_shap(
    ...
    """
    shap_exp, _ = get_or_create_explainers(model)
    ...
    try:
        ...
        # In newer shap versions, tree_explainer output for binary is [shap_for_class0, shap_for_class1] or (1, features, 2)
        ...
        base_val = float(shap_exp.expected_value[1]) if (isinstance(shap_exp.expected_value, (list, np.ndarray)) and len(shap_exp.expected_value) > 1) else float(shap_exp.expected_value or 0.5)
    except Exception as e:
        print(f"[-] SHAP computation error: {e}, using heuristic feature attribution")
        # Fallback feature heuristic weights based on feature values
        base_val = 0.5
        class1_shap = []
...
def explain_prediction_lime(
    ...
    """
    Computes LIME local instance feature breakdown.
    """
    _, lime_exp = get_or_create_explainers(model)
    ...
    if not contributions:
        # Fallback to feature rule attribution
